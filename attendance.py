...
def removeOverlap(attd):
    ...
    for i in attd :
        ...
        if i[1] =='O' and i[2] == 'O':
            i.append('Attend')
        elif i[1] == 'O' and i[2] =='X':
            i.append('Tardy')
        elif i[1]=='X' and i[2] == 'X':
            i.append('absent')
    for i in attd:
        if len(i) > 4:
            del i[4:]
    attd = sorted(list(set(map(tuple,attd))))
    return attd

def exportAsExecel(date,attd) :
    attd = removeOverlap(attd)
    attendence = [i[3] for i  in attd]
    print('Attend :',attendence.count('Attend'))
    print('Tardy : ', attendence.count('Tardy'))
    print('Absent : ',attendence.count('Absent'))
    atd = pd.DataFrame(attd,columns=['Student','first class','Second class','attendance'])
    print(atd.head())
